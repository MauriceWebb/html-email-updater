...
function styleCssToObj($) {
    const styles = {};
    getAllDocumentRules($).forEach((style) => {
        ...
        validSelectors.forEach((selector) => {
            if (styles[selector]) {
                styles[selector] = [...styles[selector], ...decs];
            } else {
    ...
    Object.keys(styleObj)
        ...
        .forEach((selector) => {
            ...
            els.each((idx, el) => {
                ...
                const proposedStyleObj = getDeclarationsObj(styleObj[selector]);
...
function getElStyleObj(el) {
    ...
    const elStyleObj = elStyles.reduce((obj, style) => {
        ...
        if (!hasValue(property)) return obj;
...
function getStyleObjString(updatedStylesObj) {
    return Object.keys(updatedStylesObj)
        .reduce((styleStr, property) => {
        ...
        .forEach((style) => {
            if (style.type === 'media') {
                const mediaRules = style.rules.reduce((str, rule) => {
                    ...
                    if (type !== 'rule') return str;
                    const decStr = declarations.map(({ property, value }) =>
                ...
                if (mediaRules) {
                    ...
                    queries.push(`@media ${style.media} {\n${finalMediaRules}\n}`);
                }
            }
        });
    return queries.join('\n');
...
async function outputHTML(htmlString) {
    ...
    const outputPath =
        argv['o'] || argv['output'] || argv['t'] || argv['template'];
    const prettyHtml = prettier.format(htmlString.trim().replace(/></g, '>\n<'), {
        ...prettierConfig,
        parser: 'html'
    })
    fse.outputFileSync(
        outputPath,
        argv['m'] || argv['--minify'] ? minify(prettyHtml, {
            html5: true,
